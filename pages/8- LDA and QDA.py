...
# --- Enforce more than 2 classes (binary → use dedicated page) ---
n_classes_check = df[y_variable].nunique(dropna=True)
if n_classes_check == 2:
    st.error(
        f"This page is for **multiclass problems** (3 or more classes). "
        f"The selected target **{y_variable}** has **exactly 2 classes**. "
        "Use the **LDA Binary (2 classes)** page instead."
    )
    st.stop()
if n_classes_check < 2:
    st.error(
        f"The selected target **{y_variable}** has fewer than 2 classes. "
        "Cannot perform discriminant analysis."
    )
    st.stop()
...
st.info(
    "**Note:** This page is intended for **3 or more classes**. "
    "For binary classification (exactly 2 classes), use the dedicated "
    "**LDA Binary (2 classes)** page."
)
...
with tab_train:
    ...
    if st.button(
        ...
        # Store in session state (keys must NOT match widget keys)
