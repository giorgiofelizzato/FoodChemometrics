...
LDA_BIN_KEYS = [
    ...
    "lda_bin_X_vars",
    "lda_bin_y_var",
    "lda_bin_n_components",
    "lda_bin_train_idx",
    "lda_bin_train_results",
    "lda_bin_test_results",
]
...
def build_project():
    ...
    project = {
        ...
        # =================================================
        # DISCRIMINANT ANALYSIS (LDA / QDA multiclass)
        # =================================================
        ...
        # =================================================
        # LDA BINARY (2 classes)
        # =================================================
        "lda_binary": {
            "lda_bin_model": _safe_get("lda_bin_model"),
            "lda_bin_X_vars": _safe_get("lda_bin_X_vars"),
            "lda_bin_y_var": _safe_get("lda_bin_y_var"),
            "lda_bin_n_components": _safe_get("lda_bin_n_components"),
            "lda_bin_train_idx": _safe_get("lda_bin_train_idx"),
            "lda_bin_train_results": _safe_get("lda_bin_train_results"),
            "lda_bin_test_results": _safe_get("lda_bin_test_results"),
        },
        # =================================================
        # PLS-DA
        # =================================================
...
def restore_project(project):
    ...
    # LDA BINARY (2 classes)
    # -------------------------------------------------
    lda_bin = project.get("lda_binary", {})
    for key in LDA_BIN_KEYS:
        val = lda_bin.get(key)
        if val is not None:
            st.session_state[key] = val
        elif key in st.session_state:
...
if "da_model" in st.session_state:
    ...
    st.info(f"Discriminant model fitted: **{mtype}** (multiclass)")

# --- LDA Binary model ---
if "lda_bin_model" in st.session_state:
    st.info("LDA Binary model fitted (2 classes)")
...
# =====================================================
# DA STATUS (multiclass)
# =====================================================

if "da_model" in st.session_state:
    st.divider()
    st.subheader("Discriminant Analysis (multiclass)")
...
if "lda_bin_model" in st.session_state:
    st.divider()
    st.subheader("LDA Binary (2 classes)")

    lda_bin_rows = [
        ("X variables", len(_safe_get("lda_bin_X_vars") or [])),
        ("Target", _safe_get("lda_bin_y_var")),
        ("Components", _safe_get("lda_bin_n_components")),
        (
            "Train results",
            "Yes" if "lda_bin_train_results" in st.session_state else "No",
        ),
        (
            "Test results",
            "Yes" if "lda_bin_test_results" in st.session_state else "No",
        ),
    ]
    st.dataframe(
        pd.DataFrame(lda_bin_rows, columns=["Item", "Value"]),
        hide_index=True,
        use_container_width=True,
    )
...
- LDA / QDA multiclass model and results (if fitted)
- LDA Binary (2 classes) model and results (if fitted)
...
if uploaded_project is not None:
    try:
        ...
        if da_sec.get("da_model") is not None:
            st.write(
                "**Discriminant model (multiclass):**",
                da_sec.get("da_model_type", "fitted"),
            )

        # LDA Binary
        lda_bin_sec = loaded_project.get("lda_binary", {})
        if lda_bin_sec.get("lda_bin_model") is not None:
            st.write(
                "**LDA Binary model:**",
                "fitted (2 classes)",
            )
