...
with col2:
    ...
    max_comp = min(len(selected_X), n_classes - 1)
    ...
    if model_type == "LDA":
        ...
        if max_comp < 1:
            ...
            st.error(
                ...
                "LDA requires at least two classes in the target variable."
        ...
        if max_comp == 1:
            ...
            # Binary classification has only one possible discriminant axis.

            # We avoid the slider here because a previous Streamlit session-state

            # value could otherwise be outside the new range.

            n_components = 1

            st.caption(

                "Binary classification: LDA provides one discriminant axis (LD1)."

            )

        else:

            previous_ncomp = st.session_state.get("lda_ncomp", 2)

            previous_ncomp = min(max(1, previous_ncomp), max_comp)
