...
if not source_options:
    st.warning("No dataset available. Load data in **Data Import** first.")
    st.stop()
...
if data_source == "Train / Test split":
    ...
    st.success(f"Preprocessed dataset: {df.shape[0]} × {df.shape[1]}")
else:
    df = st.session_state["dataset"].copy()
    st.success(f"Raw dataset: {df.shape[0]} × {df.shape[1]}")
...
y_candidates = sorted(list(set(categorical_variables + discrete_numeric)))
...
with tab_train:
    ...
    if (
        ...
        st.plotly_chart(
            plot_confusion_matrix(cm_df, "Confusion matrix — Training"),
        ...
        if res.get("cv") is not None:
            ...
            cm_cv, _ = get_confusion_df(y_true, y_pred_cv, labels)
            st.plotly_chart(
                plot_confusion_matrix(cm_cv, "Confusion matrix — Cross-validation"),
                use_container_width=True,
            )
        ...
        if model_type == "LDA" and res["scores"] is not None:
            ...
            if hasattr(model, "scalings_") and model.scalings_ is not None:
                n_ax = min(n_comp, model.scalings_.shape[1])
        ...
        with pd.ExcelWriter(buf, engine="openpyxl") as writer:
            ...
            if res.get("cv") is not None:
                ...
                cv_pred_df.to_excel(writer, sheet_name="CV_Predictions", index=False)
...
with tab_test:
    ...
    if "da_test_results" in st.session_state:
        ...
        labels_te = sorted(set(y_true_te) | set(y_pred_te))

        st.divider()
        st.subheader("📊 Test performance")

        met_te, acc_te = metrics_table(y_true_te, y_pred_te, labels_te)
        c1, c2 = st.columns([1, 3])
        with c1:
            st.metric("Test accuracy", f"{acc_te:.3f}")
        with c2:
            st.dataframe(met_te, hide_index=True, use_container_width=True)

        cm_te, _ = get_confusion_df(y_true_te, y_pred_te, labels_te)
        st.plotly_chart(
            plot_confusion_matrix(cm_te, "Confusion matrix — Test"),
            use_container_width=True,
        )
        ...
        if tres["proba"] is not None:
            ...
            proba_df = pd.DataFrame(
                tres["proba"],
                columns=[f"P({c})" for c in tres["classes"]],
                index=tres["index"],
            )
        ...
        buf_te = io.BytesIO()
        with pd.ExcelWriter(buf_te, engine="openpyxl") as writer:
            pred_te_df = pd.DataFrame({
                "Sample": np.asarray(tres["index"]).astype(str),
                "True": y_true_te.values,
                "Predicted": y_pred_te,
            })
            pred_te_df.to_excel(writer, sheet_name="Test_Predictions", index=False)
            met_te.to_excel(writer, sheet_name="Test_Metrics", index=False)
            cm_te.to_excel(writer, sheet_name="Test_CM")

            if tres["proba"] is not None:
                proba_df.to_excel(writer, sheet_name="Probabilities")

            if scores_te_df is not None:
                scores_te_df.to_excel(writer, sheet_name="LDA_Scores_Test")
